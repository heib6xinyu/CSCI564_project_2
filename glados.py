...
#TODO: for some reason asking for input doesn't work so for now I am going to use random
    dis_glados_to_queeg=2
    dis_glados_to_rhea=4
    routing_table.update({queeg:[mask,queeg,dis_glados_to_queeg]})#format: dest, subnetmask, nexthop, dist
    routing_table.update({rhea:[mask,rhea,dis_glados_to_rhea]})#format: dest, subnetmask, nexthop, dist
...
def main():
    #start
    print("Time x ", time.strftime("%H:%M:%S"), ", glados", ME)
    #set up
    createRoutingTable()
    with socket(AF_INET, SOCK_DGRAM) as server:
        print("Time x ",time.strftime("%H:%M:%S"),f', Bingding to glados {ME}:5050')
        server.connect((ME, PORT))
        inputs=[server]
        print("Time x ",time.strftime("%H:%M:%S"),f', Listening on glados {ME}:5050')
