...
CONNECTED = set()
NEW_CONNECTIONS = set()
neighbors.add(queeg)#queeg
neighbors.add(rhea)#rhea
...
#TODO: for some reason asking for input doesn't work so for now I am going to hard code
    dis_comet_to_queeg=10
    dis_comet_to_rhea=7
    routing_table.update({queeg:[mask,queeg,dis_comet_to_queeg]})#format: dest, subnetmask, nexthop, dist
    routing_table.update({rhea:[mask,rhea,dis_comet_to_rhea]})#format: dest, subnetmask, nexthop, dist
...
def main():
    #start
    print("Time x ", time.strftime("%H:%M:%S"), ", comet", ME)
    #set up
    createRoutingTable()
    with socket(AF_INET, SOCK_DGRAM) as server:
        print("Time x ",time.strftime("%H:%M:%S"),f', Connecting to comet {ME}:5050')
        server.connect((ME, PORT))
        inputs=[server]
        print("Time x ",time.strftime("%H:%M:%S"),f', Listening on comet {ME}:5050')
