...
#TODO: for some reason asking for input doesn't work so for now I am going to use random
    dis_queeg_to_comet=10
    dis_queeg_to_glados=2
    routing_table.update({comet:[mask,comet,dis_queeg_to_comet]})#format: dest, subnetmask, nexthop, dist
    routing_table.update({glados:[mask,glados,dis_queeg_to_glados]})#format: dest, subnetmask, nexthop, dist
...
def main():
    #start
    print("Time x ", time.strftime("%H:%M:%S"), ", queeg", ME)
    #set up
    createRoutingTable()
    with socket(AF_INET, SOCK_DGRAM) as server:
        print("Time x ",time.strftime("%H:%M:%S"),f', Connecting to queeg {ME}:5050')
        server.connect((ME, PORT))
        inputs=[server]
        print("Time x ",time.strftime("%H:%M:%S"),f', Listening on queeg {ME}:5050')
